...
import { ieButton } from "../buttons/ieButton"
import { twoDButton } from "../buttons/twoDButton"
import { actionButton } from "../buttons/actionButton"
const words = require('../text/words')
export const enrolletBranch = (bot: Telegraf<Scenes.SceneContext>) => {
 
    bot.hears(words.wordsAbiturient, async (ctx) => {
      await  ctx.reply(
            'Окей 🙂 Ми - кафедра Інженерного навчально-наукового інституту ім. Ю.М. Потебні ЗНУ, і ми дуже активні. 😉 Не тільки нудно навчаємось, слухаємо лекції, пишемо та захищаємо лабораторні і курсові, а ще й відвідуємо та організовуємо різні цікаві заходи. А ще ми вчимося за програмою «Подвійного диплома» з европейськими університетами.',
            enrolleButton
          );
    });
    
    bot.hears(words.wordsProgramsKafedra, async (ctx) => {
      await  ctx.reply(
          `Кого ж ми навчаємо? 
    ...
    ✅ Необхідну інформацію про освітні компоненти можна знайти за посиланнями:
    https://znuiepf.com.ua/?page_id=1643 (для бакалаврів)
    https://znuiepf.com.ua/?page_id=5404 (для магістрів)`
        );
    
        await ctx.replyWithPhoto(
          { source: 'src/images/ie.jpg' },
          ieButton
        );
      });
